...
public class SQLiteStmtTest {
    ...
    public void testUpdateDelete() throws Exception {
        Entry entry = mEntries.remove(3);
        SQLiteStmt stmt = mDb.prepare("DELETE FROM test WHERE a = ?;");
        stmt.bindLong(1, entry.mA);
        Assert.assertThat(stmt.executeUpdateDelete(), Is.is(1));
        stmt.close();

        stmt = mDb.prepare("SELECT * FROM test;");
        final SQLiteCursor cursor = stmt.executeSelect();
